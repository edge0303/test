...
export default function OnboardForm() {
  ...
  async function doLookup() {
    ...
    try {
      const json = await postJson<LookupResult & { ok: boolean; message?: string }>(
        '/api/bizno/lookup', { biz_no: bizNo });
      if (!json.ok) { setError(json.message ?? '조회에 실패했습니다.'); setLookup(null); return; }
      if (json.takenByOther) {
        setError('이미 다른 계정에 등록된 사업자등록번호입니다.'); setLookup(null); return;
      }
      setLookup(json);
  ...
  async function submit() {
    ...
    try {
      ...
      const json = await postJson<{ ok: boolean; message?: string; companyId: number }>('/api/onboard', {
        ...form,
      ...
      if (!json.ok) { setError(json.message ?? '저장에 실패했습니다.'); return; }
